...
long nanodiff(timeval* a, timeval* b)
{
    long sdiff = (long)((a->tv_sec - b->tv_sec) * 1.0E+09);
    return sdiff + (a->tv_usec - b->tv_usec);
}

void sleep_for(timeval* then, long sleep_value)
{
    timeval now;
    if (clock_gettime(CLOCK_REALTIME, (timeval*)(&now)) == -1) {
...
char* randdata(size_t length)
{
    ...
    size_t n = 0;
    if (length) {
        random_data = (char*)malloc(sizeof(char) * (length));
        if (random_data) {
            for (n = 0; n < length; ++n) {
